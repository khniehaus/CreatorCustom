...
class ExampleTaskView(gui3d.TaskView):
    ...
    def __init__(self, category):
        ...
        self.yVal = 0
        self.aTextUpdate = box.addWidget(gui.TextView('y is 0'))

        @self.mhEvent
        def onMouseDragged(event):
            self.yVal += event.y
            self.aTextUpdate = box.addWidget(gui.TextView('y is %d', self.yVal))

        self.aButton = box.addWidget(gui.Button('Button'))
        
        self.pushed = 0
        self.aButtonLabel1 = box.addWidget(gui.TextView('Pushed 0 times'))

        @self.aButton.mhEvent
        def onClicked(event):
            self.pushed += 1
            self.aButtonLabel1.setTextFormat('Pushed %d times', self.pushed)
