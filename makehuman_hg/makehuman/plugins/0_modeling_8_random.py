...
def randomize(human, symmetry):
    ...
    #                      'cheek']
    # if body:
    modifierGroups = modifierGroups + ['macrodetails', 'macrodetails-universal', 'pelvis', 'hip', 'armslegs', 'stomach', 'breast', 'buttocks', 'torso', 'eyebrows', 'eyes', 'chin', 'forehead', 'head', 'mouth', 'nose', 'neck', 'ears', 'cheek']
    ...
    for m in modifiers:
        if m.fullName not in randomValues:
            ...
            if m.fullName in ["macrodetails/Gender"]:
                sigma = 0.1
    ...
    #     # all pregnancy for male, too young or too old subjects
    #     if m.fullName in ["stomach/stomach-pregnant-decr|incr"]:
    #         sigma = 0.4
...
def getRandomValue(minValue, maxValue, middleValue, sigmaFactor = 0.7):
