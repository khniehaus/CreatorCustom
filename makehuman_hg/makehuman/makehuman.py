...
def get_revision_hg_info():
    ...
    hgRoot = getHgRoot()
    output = subprocess.Popen(["hg", "-q", "parents", "--template", "{rev}:{node|short}"], stdout=subprocess.PIPE,
                              stderr=sys.stderr, cwd=hgRoot).communicate()[0]
    output = output.strip().split(':')
    rev = output[0].strip().replace('+', '')
    revid = output[1].strip().replace('+', '')
    try:
        branch = \
        subprocess.Popen(["hg", "-q", "branch"], stdout=subprocess.PIPE, stderr=sys.stderr, cwd=hgRoot).communicate()[
            0].replace('\n', '').strip()
    except:
        branch = None
...
def get_hg_revision_1():
    ...
    try:
        ...
        print >> sys.stderr, u"NOTICE: Failed to get hg version number from command line: " + format(
            unicode(e)) + u" (This is just a head's up, not a critical error)"
    ...
    try:
        ...
        print >> sys.stderr, u"NOTICE: Failed to get hg version number using hglib: " + format(
            unicode(e)) + u" (This is just a head's up, not a critical error)"
    ...
    try:
        ...
        print >> sys.stderr, u"NOTICE: Failed to get hg parent version from dirstate file: " + format(
            unicode(e)) + u" (This is just a head's up, not a critical error)"
    ...
    try:
        ...
        print >> sys.stderr, u"NOTICE: Failed to get hg tip version from cache file: " + format(
            unicode(e)) + u" (This is just a head's up, not a critical error)"
...
def parse_arguments():
    ...
    parser.add_argument("--nomultisampling", action="store_true",
                        help="disable multisampling (used for anti-aliasing and alpha-to-coverage transparency rendering)")
...
def getThirdPartyLicenses(richtext=False):
    ...
    def _error(text):
        if richtext:
            return '<span style="color: #f5207c;">%s</span>' % text
        else:
            return text

    def _block(text):
        if richtext:
            return '%s<hr style="border: 1px solid #f5207c;">' % text
...
makes use of.\n"""
    ...
    if not os.path.isdir(license_folder):
        return result + _error(
            "Error: external licenses folder is not found, this is an incomplete CreatorCustom distribution!")
    external_licenses = [("PyQt4", ("pyQt4-license.txt", "http://www.riverbankcomputing.co.uk", "GPLv3")),
                         ...
                         ("Transformations",
                          ("transformations-license.txt", "http://www.lfd.uci.edu/~gohlke/", "BSD (3-clause)")),
                         ("pyFBX", ("pyFbx-license.txt",
    ...
    for name, (lic_file, url, lic_type) in external_licenses.items():
        ...
        if not os.path.isfile(lfile):
            result += "\n%s\n" % _error(
                "Error: License file %s is not found, this is an incomplete CreatorCustom distribution!" % lfile)
            continue
        f = open(lfile, encoding='utf-8')
        text = f.read()
        f.close()
